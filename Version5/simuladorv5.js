...
// ahora este objeto puede crecer dinámicamente
const programas = {
...
document.addEventListener("DOMContentLoaded", () => {
    actualizar_selector_programas(); // se refactoriza para poder llamarla más tarde
...
// -- NUEVA FUNCION para agregar programas personalizados --
function agregar_programa_personalizado() {
    ...
    // se crea una clave única para evitar colisiones en el objeto
    const clave = nombre.toLowerCase().replace(/\s/g, '') + Date.now();

    // se añade el nuevo programa al objeto global
    programas[clave] = {
        nombre: nombre,
        tamano: tamano_mib * 1024 * 1024
    };

    // se actualiza el menú desplegable con el nuevo programa
    actualizar_selector_programas();

    // se limpian los campos de entrada para facilitar el uso
    nombre_input.value = "";
    tamano_input.value = "";

    // se selecciona automáticamente el programa recién añadido
    document.getElementById("programaSelect").value = clave;
}

// -- funciones de utilidad (helpers) --

// se crea esta función para poder reutilizar el código de llenado del selector
function actualizar_selector_programas() {
    ...
    // si había algo seleccionado, se intenta mantener la selección
...
function dibujar_tabla_particiones() {
    ...
    memoria.forEach((bloque, indice) => {
        ...
        if (metodo_actual === "fijo" || metodo_actual === "variable") {
            if (!bloque.libre) {
...
function reiniciar_memoria() {
    ...
    const es_dinamico = metodo === "dinamico" || metodo === "compactacion";
...
function agregar_programa() {
    ...
    if (metodo === "fijo" || metodo === "variable") {
        ...
        if (!intentar_asignar_dinamica(pid, tamano_proceso)) {
            console.log("no hay espacio, intentando compactar...");
            compactar_memoria();
            actualizar_ui(); // mostramos el resultado de la compactación
            if (!intentar_asignar_dinamica(pid, tamano_proceso)) {
...
window.iniciar_compactacion = iniciar_compactacion;
window.agregar_programa_personalizado = agregar_programa_personalizado;
